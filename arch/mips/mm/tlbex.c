...
static __cpuinit int __attribute__((unused)) r45k_bvahwbug(void)
{
	/* XXX: We should probe for the presence of this bug, but we don't. */
	return 0;
}

static __cpuinit int __attribute__((unused)) r4k_250MHZhwbug(void)
{
	/* XXX: We should probe for the presence of this bug, but we don't. */
	return 0;
}

static __cpuinit int __attribute__((unused)) bcm1250_m3_war(void)
{
	return BCM1250_M3_WAR;
}

static __cpuinit int __attribute__((unused)) r10000_llsc_war(void)
{
...
#ifdef CONFIG_64BIT
static __cpuinit int __attribute__((unused)) in_compat_space_p(long addr)
{
	/* Is this address in 32bit compat space? */
	return (((addr) & 0xffffffff00000000L) == 0xffffffff00000000L);
}

static __cpuinit int __attribute__((unused)) rel_highest(long val)
{
	return ((((val + 0x800080008000L) >> 48) & 0xffff) ^ 0x8000) - 0x8000;
}

static __cpuinit int __attribute__((unused)) rel_higher(long val)
{
...
static __cpuinit void __attribute__((unused)) i_LA(u32 **buf, unsigned int rs,
						long addr)
{
...
static __cpuinit int __attribute__((unused)) insn_has_bdelay(struct reloc *rel,
							  u32 *addr)
{
	for (; rel->lab != label_invalid; rel++) {
		if (rel->addr == addr
...
static void __cpuinit __attribute__((unused))
...
static void __cpuinit __attribute__((unused)) il_b(u32 **p, struct reloc **r,
					 enum label_id l)
{
	r_mips_pc16(r, *p, l);
	i_b(p, 0);
 ...
 */
static __cpuinit void __attribute__((unused)) build_tlb_probe_entry(u32 **p)
{
 ...
 */
static __cpuinit void __attribute__((unused))
