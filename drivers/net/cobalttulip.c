...
static const char *version = "tulip.c:v0.76 4/22/97 becker@cesdis.gsfc.nasa.gov\n";
...
static struct tulip_chip_table {
  ...
  { PCI_DEVICE_ID_DEC_TULIP, "21040", 0, tulip_timer },
  { PCI_DEVICE_ID_DEC_TULIP_PLUS, "21041", 0, tulip_timer },
  { PCI_DEVICE_ID_DEC_TULIP_FAST, "21140", 0, tulip_timer }, /* + 21140A*/
  { PCI_DEVICE_ID_DEC_TULIP_21142, "21142", 0, tulip_timer }, /* + 21143 */
  {0, 0, 0, 0},
};
								   ...
								   int chip_id, int options)
{
	...
	dev = init_etherdev(dev, sizeof(struct tulip_private));

	printk("%s: DEC %s Tulip at %#3x,",
	...
	flush_cache_pre_dma_out(tp, sizeof(*tp));
#if defined(__mips__)
	dev->priv = CACHED_TO_UNCACHED(tp);
#else
	dev->priv = tp;
#endif
...
static struct fixups {
  ...
  {"Cobalt 27", 0, 0x40, 0x10, {0x1e00, 0x0000,0x009,0x011f,
								0x0380, 0x0000,
								0x0000, 0x2800, }},
  {0, 0, 0, 0, {}}};
...
static void parse_eeprom(struct device *dev)
{
...

	{
	  ...
	  if (done_did_that++ == 0)
		...
		printk(	   "  The following verbose information is emitted for\n");
		printk(	   "  bug reports on media selection.\n");
	}
	tp->mtable = 0;
...
#if defined(__mips__)
	ee_data[19] = 1;		// One controller... TJS
#endif
	...
	for (i = 0; i < 8; i ++)
	  if (ee_data[i] != ee_data[16+i])
		break                                                                                                                     ;
#if defined(__mips__)
	if ( 1 ) {			// Our EEPROM is REALLY broken
#else
	if (i >= 8) {
#endif
	  if (ee_data[0] == 0xff) {
...
#if defined(__mips__)
		// TJS Skip over this... we'll fix this later.
		p++;
#endif
...
#if defined(__mips__)
	if (request_irq(dev->irq, &tulip_interrupt,SA_INTERRUPT,
...
tulip_start_xmit(struct sk_buff *skb, struct device *dev)
{
	...
	flush_cache_pre_dma_out(skb->data, skb->len);
*(volatile char *)KSEG1;
...
#endif
			netif_rx(skb);
			flush_cache_post_dma_in(bus_to_virt(lp->rx_ring[entry].buffer1), pkt_len);
			lp->stats.rx_packets++;
		}
