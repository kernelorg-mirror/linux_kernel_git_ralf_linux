...
static int      mips_dsemul (struct pt_regs *, 
			     mips_instruction,
			     vaddr_t);

/* Function which emulates a floating point instruction. */

static int      fpu_emu (struct pt_regs *,
 ...
 */
static int 
isBranchInstr (mips_instruction *i)
{
...
static int 
mips_put_word(struct pt_regs *xcp, void *va, unsigned long val)
{
	if(!user_mode(xcp)) {
...
static int 
mips_put_dword(struct pt_regs *xcp, void *va, long long val)
{
	if(!user_mode(xcp)) {
...
static int
cop1Emulate (int xcptno, struct pt_regs *xcp, struct mips_fpu_soft_struct *ctx)
{
    mips_instruction ir;
    vaddr_t emulpc;
    vaddr_t contpc;
	 ...
	 *
	 * This branch could be a COP1 branch, but in that
	 * case we would have had a trap for that instruction, 
	 * and would not come through this route.
	 *
	 * Linux MIPS branch emulator operates on context,
	 * updating the cp0_epc.
	 */
  ...
  emul:
   ...
   /* R4000+ 64-bit fpu registers */
    ...
    case lwc1_op:
	{
	    void *va = REG_TO_VA (xcp->regs[MIPSInst_RS(ir)]) 
					+ MIPSInst_SIMM(ir);
	    ctx->regs[MIPSInst_RT(ir)] = mips_get_word(xcp, va, &err);
	    fpuemuprivate.stats.loads++;
	    if (err) {
		fpuemuprivate.stats.errors++;
		return SIGBUS;
    ...
    case swc1_op:
	{
	    void *va = REG_TO_VA (xcp->regs[MIPSInst_RS(ir)]) 
					+ MIPSInst_SIMM(ir);
	    fpuemuprivate.stats.stores++;
	    if (mips_put_word(xcp, va, ctx->regs[MIPSInst_RT(ir)])) {
		fpuemuprivate.stats.errors++;
		return SIGBUS;
	    }
	}
    ...
    case ldc1_op:
	{
	    ...
	    ctx->regs[rt + 1] = mips_get_word(xcp, va + 0, &err);
	    errs += err;
	    ctx->regs[rt + 0] = mips_get_word(xcp, va + 4, &err);
	    errs += err;
#else
	    ctx->regs[rt + 0] = mips_get_word(xcp, va + 0, &err);
	    errs += err;
	    ctx->regs[rt + 1] = mips_get_word(xcp, va + 4, &err);
	    errs += err;
#endif
	    if (err)
		return SIGBUS;
	...
	case dmtc_op:
		...
		int fs = MIPSInst_RD(ir);
		if (!(xcp->cp0_status & ST0_FR)) fs &= ~1;
		value = (MIPSInst_RT(ir)== 0) ? 0 : xcp->regs[MIPSInst_RT(ir)];
		ctx->regs[fs] = value;
	    }
	    break;
#endif
	...
	case mfc_op:
	    ...
	    if (MIPSInst_RT(ir) != 0)
	    {
		  ...
		  if (!(xcp->cp0_status & ST0_FR)) {
		    ...
		    value = ctx->regs[MIPSInst_RD(ir) & ~1] >> 32;
		  }
#endif
	      } 
	...
	case mtc_op:
		...
		if (MIPSInst_RT(ir) == 0)
		  ...
		  value = (unsigned int) xcp->regs[MIPSInst_RT(ir)];
		if (MIPSInst_RD(ir) & 1) {
#ifdef SINGLE_ONLY_FPU
		  /* illegal register in single-float mode */
		  return SIGILL;
		  ...
		  if (!(xcp->cp0_status & ST0_FR)) {
		    /* move to m.s. 32 bits */
		    ctx->regs[(MIPSInst_RD(ir) & ~1)] &= 0xffffffff;
		    ctx->regs[(MIPSInst_RD(ir) & ~1)] |= value << 32;
		    break;
		  }
#endif
	        }
		/* move to l.s. 32 bits */
		ctx->regs[MIPSInst_RD(ir)] &= ~0xffffffffLL;
	...
	case mfc_op:
	    ...
	    if (MIPSInst_RT(ir) != 0)
	    {
		unsigned        value = ctx->regs[MIPSInst_RD(ir)];
		xcp->regs[MIPSInst_RT(ir)] = value;
	    }
	    break;
	...
	case cfc_op:
		...
		if (MIPSInst_RD(ir) == FPCREG_CSR) {
		    value = ctx->sr;
#ifdef CSRTRACE
		    _mon_printf ("%p gpr[%d]<-csr=%08x\n",
			     REG_TO_VA (xcp->cp0_epc), MIPSInst_RT(ir), value);
#endif
	...
	case bc_op:
		...
		if (cond) {
		    ...
		    xcp->cp0_epc += 4;
		    contpc = REG_TO_VA xcp->cp0_epc + (MIPSInst_SIMM(ir) << 2);
...
do_dsemulret(struct pt_regs *xcp)
{
     ...
     * ASSUMES that sr unchanged by delay slot instruction,
     * which should be true in user mode, at least.
     */
    xcp->cp0_status = dsemul_sr;
    /*
     * Clear the flag that got us here.
     */
    current->tss.mflags &= ~MF_DSEMUL;
    return 0;
}


#define AdELOAD 0x8fa00001	/* lw $0,1(sp) */
...
mips_dsemul (struct pt_regs *xcp, mips_instruction ir, vaddr_t cpc)
{
    ...
    /* icache containing the dsemulret instructons, but there's no */
...
static int
fpux_emu (struct pt_regs *xcp, struct mips_fpu_soft_struct *ctx, mips_instruction ir)
{
	...
	case lwxc1_op:
	{
	    void *va = REG_TO_VA (xcp->regs[MIPSInst_FR(ir)] + 
	    ...
	    if (xcp->cp0_status & ST0_FR) {
	      ...
	      return SIGILL;
#else
	      ctx->regs[(MIPSInst_FD(ir) & ~1)] &= 0xffffffff;
	      ctx->regs[(MIPSInst_FD(ir) & ~1)] |= val << 32;
#endif
	    } else {
	      /* load to l.s. 32 bits */
	      ctx->regs[MIPSInst_FD(ir)] &= ~0xffffffffLL;
	      ctx->regs[MIPSInst_FD(ir)] |= val;
	    }
	}
	break;
	...
	case ldxc1_op:
	{
	    void *va = REG_TO_VA (xcp->regs[MIPSInst_FR(ir)] + 
				  xcp->regs[MIPSInst_FT(ir)]);
	    int err = 0;
	    ctx->regs[MIPSInst_FD(ir)] = mips_get_dword(xcp, va, &err);
	    if (err) {
		fpuemuprivate.stats.errors++;
		return SIGBUS;
	    }
	...
	case sdxc1_op:
	{
	    void *va = REG_TO_VA (xcp->regs[MIPSInst_FR(ir)] + 
				  xcp->regs[MIPSInst_FT(ir)]);
	    if (mips_put_dword(xcp, va, ctx->regs[MIPSInst_FS(ir)])) {
		fpuemuprivate.stats.errors++;
		return SIGBUS;
	    }
	}
 ...
 */
static int
fpu_emu (struct pt_regs *xcp, struct mips_fpu_soft_struct *ctx, mips_instruction ir)
{
...
int
fpu_emulator_cop1Handler (int xcptno, struct pt_regs *xcp)
{
    struct mips_fpu_soft_struct  *ctx = &current->tss.fpu.soft;
...
static int
cop1Patcher (int xcptno, struct pt_regs *xcp)
{
    struct mips_fpu_soft_struct  *ctx = &current->tss.fpu.soft;
    unsigned        sr;
    int             sig;
...
_cop1_init (int emulate)
{
	...
	fpuemuactive = 1;
	{
	    ...
	    for (i = 0; i < 32; i++)
		current->tss.fpu.soft.regs[i] = 0x7ff80bad00000000LL;
	}
