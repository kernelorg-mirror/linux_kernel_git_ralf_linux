 ...
 *
 * $Id: cobaltserial.c,v 1.1 1997/10/23 22:27:37 ralf Exp $
 */
 ...
 * Z8530s, but for now...
 */
#define NUM_SERIAL	1		/* Max number of ZS chips supported */
...
#define RECOVERY_DELAY	mydelay()

void mydelay(void)
{
	int ctr;
	for (ctr=0;ctr<0x160;ctr++);
}
...
int rs_init(void)
{
	...
	if (request_irq( 4, rs_interrupt, SA_INTERRUPT,"SCC", &zs_soft[0]))
				panic("cobaltserial: can't get irq %d",
				      4);
