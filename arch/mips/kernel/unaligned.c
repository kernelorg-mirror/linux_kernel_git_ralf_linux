...
void do_ade(struct pt_regs *regs)
{
	...
	adels = (((regs->cp0_cause & CAUSEF_EXCCODE) >> CAUSEB_EXCCODE) == 4) ? 'l' : 's';
#if 0
	/*
	 * I think there are no legal unaligned kernel accesses to the kernel
	 * space; we should fix them anyway.  So let's do a little panic ...
	 */
	if (is_kernel_address(badvaddr) && !user_mode(regs)) {
		show_regs(regs);
		panic("Caught ade%c exception in kernel mode accessing %08lx.", adels, badvaddr);
	}
#endif
