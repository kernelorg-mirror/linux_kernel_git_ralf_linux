 ...
 *
 * $Id: signal.c,v 1.7 1997/09/12 18:36:11 ralf Exp $
 */
...
asmlinkage int sys_sigsuspend(struct pt_regs regs)
{
	...
	if (verify_area(VERIFY_READ, uset, sizeof (*uset)))
		return -EFAULT;
	set = get_user(uset);
...
asmlinkage int sys_sigreturn(struct pt_regs regs)
{
	...
	if (verify_area(VERIFY_READ, context, sizeof(struct sigcontext)) ||
	    (regs.regs[29] & (SZREG - 1)))
		goto badframe;

	blocked = get_user(&context->sc_sigset);
	current->blocked = blocked & _BLOCKABLE;
	regs.cp0_epc = get_user(&context->sc_pc);

	/*
	 * Restore all integer registers.
	 */
	for(i = 31;i >= 0;i--) {
		reg = get_user(&context->sc_regs[i]);
		regs.regs[i] = (int) reg;
	}
	reg = get_user(&context->sc_mdhi);
	regs.hi = (int) reg;
	reg = get_user(&context->sc_mdlo);
	regs.lo = (int) reg;
                        ...
                        int signr, unsigned long oldmask)
{
	 ...
	 *         .set    reorder
	 */
	put_user(0x27bd0000 + scc_offset, &frame->code[0]);
	put_user(0x24020000 + __NR_sigreturn, &frame->code[1]);
	put_user(0x0000000c, &frame->code[2]);
	 ...
	 */
	put_user(regs->cp0_epc, &sc->sc_pc);
	put_user(regs->cp0_status, &sc->sc_status);	/* Status register */
	for(i = 31;i >= 0;i--)
		put_user(regs->regs[i], &sc->sc_regs[i]);
	save_fp_context(sc);
	put_user(regs->hi, &sc->sc_mdhi);
	put_user(regs->lo, &sc->sc_mdlo);
	put_user(regs->cp0_cause, &sc->sc_cause);
	put_user((regs->cp0_status & ST0_CU1) != 0, &sc->sc_ownedfp);
	put_user(oldmask, &sc->sc_sigset);
	put_user(0, &sc->__pad0[0]);
	put_user(0, &sc->__pad0[1]);
	put_user(0, &sc->__pad0[2]);
